...
function seed016Database(dbPath) {
  ...
  `);
  db.prepare("INSERT INTO pdv_meta (name, value) VALUES (?, ?)").run("installed-pdv-version", "0.1.16");
...
test("upgrade 0.1.16 -> 0.1.17 preserva caixa, estoque, clientes, vendas, movimentos e configurações", () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "pdv-upgrade-"));
  const dbPath = path.join(tempDir, "pdv-nexus.sqlite");
  const backupDir = path.join(tempDir, "backups");
  seed016Database(dbPath);
  const before = snapshot(dbPath);

  const result = preparePdvVersionMigration({ DatabaseSync, dbPath, backupDir, currentVersion: "0.1.17" });
  const after = snapshot(dbPath);

  assert.equal(result.status, "migrated");
  assert.equal(result.previousVersion, "0.1.16");
  assert.equal(after.version, "0.1.17");
...
test("comparação de versões só oferece versão realmente mais nova", () => {
  ...
  assert.equal(compareVersions("0.1.16", "0.1.17"), -1);
});
