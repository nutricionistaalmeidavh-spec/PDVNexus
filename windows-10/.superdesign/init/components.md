...
Shared React UI primitives used by the demo apps. The codebase uses inline `CSSProperties` rather than Tailwind or a packaged component library.

Exports:
- `AppShell`: two-column application shell with collapsible dark sidebar and top page header.
- `SectionCard`: white content card with title, optional subtitle, and optional wide grid behavior.
- `Pill`: compact gray rounded label.
- `TextInput`: full-width rounded text input.
...
export function SectionCard(props: PropsWithChildren<{ title: string; subtitle?: string; hidden?: boolean; wide?: boolean }>) {
