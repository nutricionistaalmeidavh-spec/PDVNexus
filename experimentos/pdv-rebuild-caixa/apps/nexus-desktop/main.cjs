...
// A reconstrução usa um perfil próprio. Ela nunca abre ou altera o banco da versão funcional.
app.setName("PDV Nexus Rebuild");
app.setPath("userData", path.join(app.getPath("appData"), "PDV Nexus Rebuild"));
...
function listPdvCatalog(storeKey) {
  const db = getPdvDatabase();
  const key = String(storeKey ?? "default");
  migrateLegacyPdvSnapshotIfNeeded(db, key);
  return db.prepare("SELECT payload_json AS payloadJson FROM pdv_products WHERE store_key = ? ORDER BY product_code").all(key)
    .map((row) => parsePdvJson(row.payloadJson, null))
    .filter((product) => product && typeof product === "object" && product.productCode);
}

function savePdvCatalogProduct(storeKey, product, previousProductCode) {
  if (!product || typeof product !== "object" || !String(product.productCode ?? "").trim()) {
    throw new Error("Produto inválido; informe o código do produto.");
  }
  const db = getPdvDatabase();
  const key = String(storeKey ?? "default");
  const code = String(product.productCode).trim();
  const previousCode = String(previousProductCode ?? code).trim();
  db.exec("BEGIN IMMEDIATE");
  try {
    if (previousCode && previousCode !== code) {
      db.prepare("DELETE FROM pdv_products WHERE store_key = ? AND product_code = ?").run(key, previousCode);
    }
    db.prepare(`
      INSERT INTO pdv_products (store_key, product_code, payload_json)
      VALUES (?, ?, ?)
      ON CONFLICT(store_key, product_code) DO UPDATE SET payload_json = excluded.payload_json
    `).run(key, code, JSON.stringify({ ...product, productCode: code }));
    db.exec("COMMIT");
  } catch (error) {
    db.exec("ROLLBACK");
    throw error;
  }
  return listPdvCatalog(key);
}

function removePdvCatalogProduct(storeKey, productCode) {
  const db = getPdvDatabase();
  const key = String(storeKey ?? "default");
  const code = String(productCode ?? "").trim();
  if (!code) throw new Error("Produto inválido para exclusão.");
  db.prepare("DELETE FROM pdv_products WHERE store_key = ? AND product_code = ?").run(key, code);
  return listPdvCatalog(key);
}

function replacePdvCatalog(storeKey, products) {
  if (!Array.isArray(products) || products.some((product) => !product || typeof product !== "object" || !String(product.productCode ?? "").trim())) {
    throw new Error("Catálogo inválido; cada produto precisa ter um código.");
  }
  const db = getPdvDatabase();
  const key = String(storeKey ?? "default");
  const codes = new Set(products.map((product) => String(product.productCode).trim()));
  if (codes.size !== products.length) throw new Error("Catálogo inválido; há códigos de produto repetidos.");
  db.exec("BEGIN IMMEDIATE");
  try {
    db.prepare("DELETE FROM pdv_products WHERE store_key = ?").run(key);
    ...
    for (const product of products) {
      ...
      insertProduct.run(key, productCode, JSON.stringify({ ...product, productCode }));
    }
    db.exec("COMMIT");
  } catch (error) {
    db.exec("ROLLBACK");
...
ipcMain.handle("nexus-pdv-catalog:list", (_event, storeKey) => listPdvCatalog(storeKey));
ipcMain.handle("nexus-pdv-catalog:save", (_event, storeKey, product, previousProductCode) => savePdvCatalogProduct(storeKey, product, previousProductCode));
ipcMain.handle("nexus-pdv-catalog:remove", (_event, storeKey, productCode) => removePdvCatalogProduct(storeKey, productCode));
ipcMain.handle("nexus-pdv-catalog:replace", (_event, storeKey, products) => replacePdvCatalog(storeKey, products));
