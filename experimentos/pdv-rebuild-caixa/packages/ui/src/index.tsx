...
export function AppShell(props: PropsWithChildren<{ title: string; nav: NavItem[]; collapsible?: boolean; sidebarPosition?: "left" | "right"; sidebarTitle?: string; hideTitle?: boolean; activePath?: string }>) {
  const { title, nav, children, collapsible = true, sidebarPosition = "left", sidebarTitle = "Nexus Core", hideTitle = false, activePath } = props;
  const [collapsed, setCollapsed] = useState(false);

  return (
    <div style={{ ...styles.page, gridTemplateColumns: sidebarPosition === "right" ? `minmax(0, 1fr) ${collapsed ? "68px" : "208px"}` : `${collapsed ? "68px" : "208px"} minmax(0, 1fr)` }}>
      <aside style={{ ...styles.sidebar, order: sidebarPosition === "right" ? 2 : 1, paddingInline: collapsed ? "10px" : "16px" }}>
        ...
        <nav style={styles.nav}>
          {nav.map((item) => (
            <a key={item.key} href={`#${item.path}`} aria-current={item.path === activePath ? "page" : undefined} style={{ ...styles.navItem, ...(item.path === activePath ? styles.navItemActive : {}), justifyContent: collapsed ? "center" : "flex-start" }} title={collapsed ? item.label : undefined}>
              <span>{item.icon ?? "•"}</span>
